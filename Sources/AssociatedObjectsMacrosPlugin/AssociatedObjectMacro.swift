...
fileprivate extension Diagnostic {
	...
	static func unexpectedSetAccessor(_ node: AccessorDeclSyntax) -> Self {
		DiagnosticBuilder(for: node.accessorSpecifier)
			.messageID(domain: "AssociatedObject", id: "unexpected_set_accessor")
			.message("`@AssociatedObject` does not support custom `set` accessors, use `didSet` instead")
			.build()
	}
